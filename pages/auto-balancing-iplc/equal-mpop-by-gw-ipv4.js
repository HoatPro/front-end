        ...
        return (<Table celled structured id="tblList">
            <Table.Header>
                <Table.Row>
                    ...
                    <Table.HeaderCell>Equal-Interface</Table.HeaderCell>
                    ...
                    this.props.listToDisplay.map((item, index, list) => {
                        return (
                            <Table.Row key={index}>
                                ...
                                <Table.Cell key={'f'}>{item.listIpv4.map((miniItem, index)=> {
                                ...
                                <Table.Cell key={'f'}>{item.listIpv4.map((miniItem, index)=> {
                                    return <Fragment key={index} ><b>Ipv4: </b> { miniItem.ipv4} <b> - Traffic: </b> { miniItem.traffic }<br/> </Fragment>;
                                })}</Table.Cell>
...
class EqualMpopByGwIpv4 extends Component {
    constructor(props) {
        super(props);
        this.state = {
            search: {searchByName: ''},
    ...
    componentDidMount() {
        ...
        if (_.size(this.state.equalMpopByGwDataIpv4)===0) {
            this.props.dispatch(autoBalancingIPLC.getEqualMpopByGwDataIpv4());
        }
    }

    componentWillReceiveProps(nextProps) {
        const {equalMpopByGwDataIpv4} = nextProps;
        let listData = equalMpopByGwDataIpv4;
        this.setState({
            equalMpopByGwDataIpv4: equalMpopByGwDataIpv4,
            listData: listData,
        ...
        return (
            <div id="div-page-auto-balancing-iplc">
                ...
                <DashboardLayout >
                    ...
                    <Segment>
                        <Header> Equal MPOP by GW </Header>
...
export default connect(mapStateToProps, null)(EqualMpopByGwIpv4);